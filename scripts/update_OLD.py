...
from scripts.scraper import fetch_page_content, extract_conference_details
from scripts.GPTsearchURL import search_conference_website
...
class Default(WorkerEntrypoint):
    ...
    async def perform_updates(self):
        ...
        res = await self.env.DB.prepare(
            "SELECT id, title, acronym, url FROM conferences WHERE start IS NOT NULL ORDER BY start ASC LIMIT "
        ).all()
        ...
        for conf in conferences:
            try:
                ...
                url = search_conference_website(conf.get("title"), conf.get("acronym", ""))
                
                # Only proceed if we found a NEW url
                if url and url != conf.get("url"):
                    page_content = await fetch_page_content(url) # Ensure this uses async fetch
                    if not page_content:
                        continue
