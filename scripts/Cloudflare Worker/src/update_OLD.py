...
class Default(WorkerEntrypoint):
    ...
    async def perform_updates(self):
        ...
        ") "
        "AND NOT EXISTS ("
        "  SELECT 1 FROM submitted_conferences sc "
        "  WHERE sc.id = c.id AND sc.edit_type = 'auto-update' AND sc.status = 'waiting'"
        ") "
